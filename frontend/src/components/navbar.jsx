...
export const Navbar = ({ darkMode, setDarkMode }) => {
  ...
  const NAVBAR_HEIGHT = 35; // Sticky navbar height for correct offset
    ...
    >
      ...
      <div className="relative cursor-pointer group">
        <Link
          ...
          className="text-2xl font-bold text-primary 
                     bg-clip-text  group-hover:scale-110 transition-transform duration-300"
        >
        ...
        <div className="absolute inset-x-0 top-full flex justify-start opacity-50">
          <span
            className="text-2xl font-bold text-primary/30
                       bg-clip-text blur-sm select-none transform scale-y-[-1]"
          >
            Sakib<span className="text-accent/30">.</span>
          </span>
        </div>
      </div>
      ...
      <nav className="hidden md:flex gap-8 items-center">
        {links.map((link) => (
          <Link
            ...
            className="relative cursor-pointer transition-colors duration-300 
                       ...
                       hover:after:w-full after:transition-all after:duration-300 font-medium"
          >
            {link.label}
          </Link>
        ))}
        ...
        <Link
          ...
          spy={true}
          offset={-NAVBAR_HEIGHT}
          className="bg-primary text-white px-3 py-1.5 rounded-xl shadow-md font-medium
        ...
        >
          ...
          <Link
            ...
            className="bg-primary text-white px-4 py-1.5 rounded-xl shadow-md 
                       hover:bg-primary-dark hover:shadow-lg transition-all duration-300 cursor-pointer font-medium"
            onClick={() => setOpen(false)}
          >
            Let's connect
          </Link>
